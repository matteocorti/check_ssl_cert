...
# Copyright (c) 2007-2026 Matteo Corti <matteo@corti.li>
#
...
SIGNALS="HUP INT QUIT TERM ABRT EXIT"
...
usage() {
    ...
    echo "      --assume-online              Do not check if the port is open"
    # Delimiter at 78 chars ############################################################
    ...
    echo "      --ignore-crl                 Ignore CRLs"
    echo "      --ignore-dh                  Ignore too small DH keys"
    ...
    echo "   -p,--port port                  TCP port number (default 443)"
    ...
    echo "      --require-no-tls1_2          Critical if TLS 1.2 is offered"
    ...
    echo "                                   (can be specified multiple times)"
    echo "      --crl                        Check revocation via CRL (enabled by"
    echo "                                   default)"
...
debuglog() {
    ...
    if [ -n "${DEBUG_TIME}" ]; then
        NOW=$(date +%s)
        DBG_ELAPSED=$((NOW - DEBUG_TIME))
        DBG_ELAPSED=$(printf "%04d" "${DBG_ELAPSED}")
    fi
    ...
    if [ "${LEVEL}" -le "${DEBUG}" ]; then
        if [ -n "${DBG_ELAPSED}" ]; then
            echo "${1}" | sed "s/^/[DBG ${DBG_ELAPSED}s] /" >&2
        else
            echo "${1}" | sed "s/^/[DBG] /" >&2
        fi
    fi
...
EOT
        ...
        debuglog "expect or timeout not available: disabling timeouts"
...
################################################################################
# Fetches the issuer cert for a given certificate element
# Params
#   $1 certificate element
# Stores the fetched  certificate in ISSUER_CERT
fetch_issuer_cert() {
    ...
    if [ -z "${ELEMENT_ISSUER_URIS}" ]; then
        verboselog "Warning cannot find the CA Issuers in the certificate chain element ${el_number}: disabling revokation checks on chain element ${el_number}"
        return
    fi
    ...
    for ELEMENT_ISSUER_URI in ${ELEMENT_ISSUER_URIS}; do
        ...
        if [ -z "${ISSUER_CERT}" ]; then
            ...
            debuglog "Fetching issuer certificate ${ELEMENT_ISSUER_URI} to ${ISSUER_CERT_TMP}"

            TIMEOUT_REASON="Fetching issuer ${ELEMENT_ISSUER_URI}"
            ...
            debuglog "Issuer certificate type (1): ${TYPE_TMP}"

            if echo "${ELEMENT_ISSUER_URI}" | "${GREP_BIN}" -F -q 'p7c'; then
                debuglog "Converting issuer certificate from PKCS #7 to PEM"
            ...
            TYPE_TMP="$(${FILE_BIN} -L -b "${ISSUER_CERT_TMP}" | sed 's/.*://')"
            debuglog "Issuer certificate type (2): ${TYPE_TMP}"

            # check for errors
            if "${FILE_BIN}" -L -b "${ISSUER_CERT_TMP}" | "${GREP_BIN}" -E -q HTML; then
                debuglog "HTML page returned instead of a certificate"
            ...
            if ! "${FILE_BIN}" -L -b "${ISSUER_CERT_TMP}" | "${GREP_BIN}" -E -q '(ASCII|PEM)'; then
                ...
                if "${FILE_BIN}" -L -b "${ISSUER_CERT_TMP}" | "${GREP_BIN}" -E -q '(data|Certificate)'; then
                    ...
                    debuglog "Converting issuer certificate from DER to PEM"
                    ...
                    debuglog "Empty issuer certificate detected: skipping"
                    return

                else

                    TYPE_TMP="$(${FILE_BIN} -L -b "${ISSUER_CERT_TMP}")"
                    debuglog "Complete issuer certificate type ${TYPE_TMP}"
            ...
            TYPE_TMP="$(${FILE_BIN} -L -b "${ISSUER_CERT_TMP}" | sed 's/.*://')"
            debuglog "Issuer certificate type (3): ${TYPE_TMP}"
            ...
            if [ -n "${DEBUG_CERT}" ]; then
                ...
                debuglog "Storing a copy of the retrieved issuer certificate to ${FILE_NAME} for debugging purposes"
...
check_crl() {
    ...
    debuglog "Checking CRL for element ${el_number}"
    subject=$( extract_cert_attribute 'subject' "${CERT_ELEMENT}")
    debuglog "Checking CRL for element ${el_number} (${subject})"
    ...
    if [ -n "${CRL_URI}" ]; then
        ...
        SERIAL="$(extract_cert_attribute 'serial' "${CERT_ELEMENT}")"
        debuglog "Checking if the element serial number (${SERIAL}) is contained in the CRL"


        if openssl crl -in "${CRL_TMP_PEM}" -text 2>/dev/null | grep -q "Serial Number: ${SERIAL}$" ; then
...
check_ocsp() {
    ...
    if [ -n "${OCSP}" ]; then
        ...
        fetch_issuer_cert "${CERT_ELEMENT}"

        if ! [ -r "${ISSUER_CERT}" ] ; then
            verboselog "Unable to fetch the issuer certificate: skipping OCSP check"
            return
        fi
...
################################################################################
# Checks if a certificate expires within the next $2 seconds
#   $1 cert
#   $2 number of seconds
# Returns
#   1 if it expires (false)
#   0 if not (true)
checkend() {

    # check if OpenSSL is buggy (see https://github.com/openssl/openssl/issues/28928)
    # the result is cached to check only once
    if [ -z "${CHECKEND_BUGGY_CHECKED}" ] ; then

        debuglog "Checking if OpenSSL checkend is buggy"

        CHECKEND_BUGGY_CHECKED=1

        if echo "${1}" |  ${OPENSSL} x509 -in /dev/stdin -noout -checkend 2147483647 >/dev/null  ; then

            # exit status 0 but should be 1

            debuglog "  OpenSSL checkend is *buggy*"
            debuglog "    fallback to parsing the text output"

            CHECKEND_BUGGY=1

        else

            debuglog "  OpenSSL checkend is not buggy"

        fi

    fi

    debuglog "${OPENSSL} x509 -in /dev/stdin -noout -checkend ${2}"

    if [ -n "${CHECKEND_BUGGY}" ] ; then

        debuglog "Checking validity parsing the output of x509 -checkend (buggy)"

        #   we fall back to parse the text output
        if echo "${1}" |
                ${OPENSSL} x509 -in /dev/stdin -noout -checkend "${2}" |
                grep -q 'Certificate will not expire' ; then
            debuglog "  Certificate will not expire"
            return 0
        else
            debuglog "  Certificate will expire"
            return 1
        fi

    else

        debuglog "Checking validity with x509 -checkend"

        if echo "${1}" |  ${OPENSSL} x509 -in /dev/stdin -noout -checkend "${2}" >/dev/null ; then
            debuglog "  RES=0"
            return 0
        else
            debuglog "  RES=1"
...
check_cert_end_date() {
    ...
    if [ -z "${EARLIEST_VALIDITY_HOURS}" ] || compare "${HOURS_UNTIL}" "<" "${EARLIEST_VALIDITY_HOURS}"; then
        ...
        debuglog "    EARLIEST_VALIDITY_HOURS=${EARLIEST_VALIDITY_HOURS}"
    fi

    if [ -z "${DAYS_VALID}" ] || compare "${ELEM_DAYS_VALID}" "<" "${DAYS_VALID}"; then
        DAYS_VALID="${ELEM_DAYS_VALID}"
        debuglog "    DAYS_VALID=${DAYS_VALID}"
    fi
...
${CN_EXPIRED_TMP}"
        ...
        if [ -n "${CRITICAL_DAYS}" ] && [ -n "${CRITICAL_SECONDS}" ]; then
            ...
            if ! checkend "${1}" "${CRITICAL_SECONDS}" ; then
...
#            if ! echo "${1}" | ${OPENSSL} x509 -in /dev/stdin -noout -checkend "${WARNING_SECONDS}" >/dev/null; then
            if ! checkend "${1}" "${WARNING_SECONDS}" ; then
...
fetch_certificate() {
    ...
    if [ "${RET}" -ne 0 ]; then
        ...
        elif ascii_grep "certificate has expired" "${ERROR}" ; then
...
parse_command_line_options() {
    ...
    while true; do
        ...
        --require-no-tls1_1)
            ...
            ;;
        --require-no-tls1_2)
            DISALLOWED_PROTOCOLS="${DISALLOWED_PROTOCOLS} TLSv1.2"
            shift
            ;;
...
$2"
        ...
        -n | --cn | -m | --match)
            ...
            # as soon as -m is specified we check altnames too
            ALTNAMES=1
        ...
        -p | --port)
            ...
            check_integer "$2" "--port: the port number should be an integer"
            PORT="$2"
            shift 2
            ;;
        --precision)
...
main() {
    ...
    if [ -z "${TIMEOUT_BIN}" ]; then
        # timeout not available: try with gtimeout
    ...
    *) ;;
    ...
    if [ -n "${ALTNAMES}" ] && [ -z "${IGNORE_HOST_CN}" ] && [ -z "${NAMES_TO_BE_CHECKED}" ] && [ "${HOST_IS_IP}" -eq 0 ]; then
        unknown "--altnames requires a common name to match (--match)"
    fi
    ...
    if ! "${DATEBIN}" +%s >/dev/null 2>&1; then
        ...
        if "${DATEBIN}" --version 2>&1 | "${GREP_BIN}" -E -q '(GNU|uutils)'; then
            DATETYPE='GNU'
    ...
    if [ -n "${REQUIRE_DNSSEC}" ]; then
        ...
        # The output of dig on Ubuntu is sometimes interpreted as binary: we are using grep -a

        DNSSEC_ERROR=
        # check for the presence of the Authenticated Data (ad) flag in the header
        if ! echo "${DIG_OUTPUT}" | "${GREP_BIN}" -a ';; flags:' | "${GREP_BIN}" -a -q 'ad[; ]'; then
        ...
        if ! echo "${DIG_OUTPUT}" | "${GREP_BIN}" -a ', flags:' | "${GREP_BIN}" -a -q 'do[; ]'; then
        ...
        if ! echo "${DIG_OUTPUT}" | "${GREP_BIN}" -a -q 'RRSIG'; then
...
"
            ...
            CERT_IN_CHAIN=$((CERT_IN_CHAIN + 1))
            if [ "${ELEMENT}" -ne 0 ] && [ $((ELEMENT - CERT_IN_CHAIN)) -lt 0 ]; then
                break
            fi
        done
